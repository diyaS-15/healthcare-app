...
def normalize_marker_name(raw_name: str) -> str:
    """
    Convert any lab-specific name to the standard name.
    Returns the raw name (lowercased, cleaned) if no match found.
    """
    key = raw_name.lower().strip()
    return _ALIAS_TO_STANDARD.get(key, key.replace(" ", "_"))

def get_display_name(standard_name: str) -> str:
    """Get the human-readable display name for a marker."""
    return DISPLAY_NAMES.get(standard_name, standard_name.replace("_", " ").title())

def normalize_markers(markers: list[dict]) -> list[dict]:
    """
    Process a list of raw marker dicts and add normalized names.
    Input format: [{"name": "Hb", "value": 12.5, "unit": "g/dL", ...}]
    """
    normalized = []
    for m in markers:
        std_name = normalize_marker_name(m["name"])
        normalized.append({
            **m,
            "normalized_name": std_name,
            "display_name": get_display_name(std_name),
        })
    return normalized
