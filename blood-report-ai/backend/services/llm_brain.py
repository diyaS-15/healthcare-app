...
DEFAULT_CHAT_MODEL = os.getenv("OPENAI_CHAT_MODEL", "").strip() or os.getenv("OPENAI_MODEL", "").strip() or "gpt-4o-mini"
DEFAULT_QUESTION_MODEL = os.getenv("OPENAI_QUESTION_MODEL", "").strip() or DEFAULT_CHAT_MODEL
...
def _error_hint(exc: Exception) -> str:
    ...
    if "model_not_found" in text or "does not exist" in text:
        return "The configured OpenAI model is unavailable for this account."
...
def general_chat(history: list = None, new_message: str = "", simple_mode: bool = False, message: str = "", context: str = "") -> str:
    ...
    try:
        ...
        return f"{_error_hint(e)} Please consult a healthcare provider for medical concerns."
...
def follow_up_response(original_analysis: str, user_answer: str) -> str:
    ...
    try:
        ...
        return f"I'd be happy to help with that follow-up question, but {_error_hint(e).lower()} Please try again."
